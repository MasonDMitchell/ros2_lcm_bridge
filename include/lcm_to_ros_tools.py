...
#NOTE The order of the variables in the ros message will be predicated on their type and then ordered by their occurance in the lcm file. Easy to fix if needed
def get_lcm_variables_as_ros(lcm_message_filepath):
    ...
    #Remove all newlines, tabs, and returns from string
    lcm_file = lcm_file.replace('\n', '').replace('\r', '').replace('\t','')
    #TODO Remove comments from file by regexing everything from '//' to '\n'. Would remove edgecase of typename in comment
...
def generate_ros_message(ros_variables,ros_message_filepath):
    with open(ros_message_filepath, mode='wt', encoding='utf-8') as ros_msg_file:
        ros_msg_file.write("#THIS IS AN AUTOGENERATED FILE DO NOT CHANGE\n")
        ros_msg_file.write('\n'.join(ros_variables))
...
def ros_message_name_to_package(name):
    return "_"+str(camel_to_snake_case(name))

if __name__ == "__main__":
    x,y = find_lcm_files()
    a=ros_message_name_to_package("EXAMPLET")
    print(a)
